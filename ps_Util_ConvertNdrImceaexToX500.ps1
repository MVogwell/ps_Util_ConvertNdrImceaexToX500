...
.PARAMETER NdrAddress
    This is the address you find in an NDR starting IMCEAEX and contain the LegacyExchangeDC address. This will look something like IMCEAEX-_o=DomainName_ou=Exchange+20Administrative+20Group+20+28FYDIBOHF23SPDLT+29_cn=Recipients_cn=aa8d1e3228c94172b0081fb5c497beca-Bob+20Hope@contoso.com
...
[CmdletBinding()]
param (
	[Parameter(Mandatory=$true)][string]$NdrAddress,
	[Parameter(Mandatory=$false)][bool]$ReturnAddressOnly = $False
)

if($ReturnAddressOnly -eq $False) {
	write-host "`n`nConvert a NDR IMCEAEX address to X500 address" -fore green
	write-host "MVogwell - v1.0 - 05-12-18 `n`n" -fore green
}

If($NdrAddress -like 'IMCEAEX-*') {


	# Replace the unrequired elements
	$NdrAddress = $NdrAddress.replace("_","/")
	$NdrAddress = $NdrAddress.replace("+20"," ")
	$NdrAddress = $NdrAddress.replace("+28","(")
	$NdrAddress = $NdrAddress.replace("+29",")")
	$NdrAddress = $NdrAddress.replace("IMCEAEX-","")

	# Remove the domain name at the end of the address following the @ symbol
	$tmpArray = $NdrAddress.split("@")
	$NdrAddress = $tmpArray[0]
	...
	if($ReturnAddressOnly -eq $False) {
		...
		write-host "The X500 address to use is: $NdrAddress `n"
		write-host "This has been copied to your clipboard`n`n"
	}
	Else {
		write-host "`n$NdrAddress" -fore cyan
	}
}
Else {
	write-host "You haven't entered a valid address!" -fore red
	write-host "The address entered for NdrAddress must start IMCEAEX-`n" -fore red
	write-host "Please try again`n`n" -fore red
}
